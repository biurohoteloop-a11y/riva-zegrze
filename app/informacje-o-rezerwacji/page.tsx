      ...
      <section className="relative bg-gradient-to-br from-[#4A6B5E] to-[#6e7a73] text-white py-24 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          ...
          <div className="flex items-center gap-4 mb-6">
            <Info className="w-12 h-12 text-[#AB8A62]" strokeWidth={1.5} />
            <div>
          ...
          <div className="space-y-12">
            ...
            >
              <div className="space-y-6">
                <div className="bg-[#f1f1ed] p-6 rounded-lg border border-[#d4d6ce]">
                  <h4 className="font-medium text-[#1a4d2e] mb-4 flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-[#AB8A62]" />
                ...
                <div className="bg-white p-6 rounded-lg border border-[#d4d6ce]">
                  <h4 className="font-medium text-[#1a4d2e] mb-3">⏱️ Natychmiastowe potwierdzenie</h4>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="bg-white p-5 rounded-lg border border-[#d4d6ce]">
                  <p className="text-sm text-[#6e7a73]">
            ...
            >
              <div className="space-y-4">
                <div className="bg-[#f1f1ed] p-6 rounded-lg border border-[#d4d6ce]">
                  <h4 className="font-medium text-[#1a4d2e] mb-3">🖥️ Polski system rezerwacyjny</h4>
                  <p className="text-sm text-[#6e7a73] mb-4">
                  ...
                  <ul className="space-y-2 text-sm text-[#6e7a73]">
                    <li className="flex items-start gap-2">
                      <span className="text-[#AB8A62] mt-1">✓</span>
                ...
                <div className="bg-white p-6 rounded-lg border border-[#d4d6ce]">
                  <h4 className="font-medium text-[#1a4d2e] mb-3">💳 Bezpieczne płatności online</h4>
                  ...
                  <div className="grid md:grid-cols-3 gap-3">
                    <PaymentFeature icon="🔒" label="Szyfrowane połączenie SSL" />
                    <PaymentFeature icon="💳" label="Karty płatnicze" />
                    <PaymentFeature icon="🏦" label="Przelewy bankowe" />
                    <PaymentFeature icon="📱" label="BLIK" />
                    <PaymentFeature icon="✅" label="Certyfikat PCI DSS" />
            ...
            >
              <div className="bg-gradient-to-br from-[#4A6B5E] to-[#6e7a73] text-white p-8 rounded-lg">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white/10 backdrop-blur-sm p-5 rounded-lg border border-white/20">
                    <div className="flex items-start gap-3 mb-3">
                      <MapPin className="w-5 h-5 text-[#AB8A62] flex-shrink-0 mt-1" />
                      <div>
                        <p className="font-medium mb-2">Adres obiektu:</p>
            ...
            >
              <div className="bg-[#f1f1ed] p-6 rounded-lg border border-[#d4d6ce]">
                ...
                <div className="mt-4 pt-4 border-t border-[#d4d6ce] flex items-center gap-4">
                  <a 
...
function TimeBox({ label, time, description }: { label: string; time: string; description: string }) {
  return (
    <div className="bg-white p-4 rounded border border-[#d4d6ce]">
      <p className="text-xs uppercase tracking-wider text-[#8a968f] mb-1">{label}</p>
      <p className="text-2xl font-light text-[#1a4d2e] mb-1" style={{ fontFamily: 'Playfair Display, serif' }}>
        {time}
      </p>
      <p className="text-xs text-[#6e7a73]">{description}</p>
    </div>
  );
}
...
function ExceptionCard({ icon, title, description }: { icon: string; title: string; description: string }) {
  return (
    <div className="bg-white p-4 rounded-lg border border-[#d4d6ce] flex items-start gap-3">
      <span className="text-2xl flex-shrink-0">{icon}</span>
      <div>
        <h4 className="font-medium text-[#1a4d2e] mb-1">{title}</h4>
...
function ContactItem({ icon, label, value, link }: { icon: React.ReactNode; label: string; value: string; link: string }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center text-[#AB8A62]">
        {icon}
      </div>
      <div>
        ...
        <a 
          href={link}
          className="text-white hover:text-[#AB8A62] transition-colors font-medium"
        >
          {value}
        </a>
      </div>
...
function LinkCard({ title, description, link, icon }: { title: string; description: string; link: string; icon: string }) {
  return (
    <a 
      href={link}
      className="bg-white p-6 rounded-lg border border-[#d4d6ce] hover:border-[#AB8A62] transition-all hover:shadow-md group"
    >
      <div className="flex items-start gap-4">
        <span className="text-3xl">{icon}</span>
        <div>
          <h3 className="font-medium text-[#1a4d2e] mb-1 group-hover:text-[#AB8A62] transition-colors">
            {title}
          </h3>
          <p className="text-sm text-[#6e7a73]">{description}</p>
        </div>
